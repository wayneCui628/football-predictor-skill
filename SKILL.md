...
2. **Execute Exhaustive Autonomous Research (NO INTERNAL MEMORY)**: You MUST proactively use your `search_web` and `read_url_content` tools to gather ALL necessary data. **NEVER rely on your internal training memory for ANY facts.** You MUST execute dedicated searches for the following:
   ...
   - **Starting Lineups & Missing Players (CRITICAL FIRST STEP)**: You MUST FIRST search for `"[Team A] vs [Team B] predicted lineups starting XI [Current Month] [Current Year]"`. You must explicitly extract the expected starting 11 for both teams. By analyzing the expected starting lineups and the accompanying "Team News" section of these articles, you will accurately identify all injured, suspended, or dropped players (e.g. Saibari). SECOND, execute individual, specific searches for the team's absolute star players (e.g. "Mbappe injury news [Current Month]") as insurance.
